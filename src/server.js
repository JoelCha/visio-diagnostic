...
function testUDP(host, port, timeoutMs = 4000) {
  return new Promise(resolve => {
    ...
    // Envoi d'un paquet STUN Binding Request minimal
    const stun = Buffer.from([
      0x00,0x01,0x00,0x00,
      0x21,0x12,0xa4,0x42,
      0x00,0x00,0x00,0x00,
      0x00,0x00,0x00,0x00,
    ...
    client.send(stun, 0, stun.length, port, host, err => {
...
const TARGETS = {
  ...
  media: [
    ...
    { label: 'Media node ICE/UDP',   host: '148.253.97.220', port: 50000, proto: 'udp' },
    { label: 'Media node ICE/UDP',   host: '142.44.63.2',    port: 50000, proto: 'udp' },
    { label: 'Media node ICE/UDP',   host: '142.44.52.178',  port: 55000, proto: 'udp' },
  ],
  coturn: [
...
app.get('/api/test', async (req, res) => {
  ...
  let total = groups.reduce((acc, [, arr]) => acc + arr.length, 0);
  let done  = 0;
  ...
  for (const [group, targets] of groups) {
    for (const t of targets) {
      let result;
      if (t.proto === 'https') {
        result = await testHTTPS(t.url);
      } else if (t.proto === 'udp') {
        result = await testUDP(t.host, t.port);
      } else {
        result = await testTCP(t.host, t.port);
      }
